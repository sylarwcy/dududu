...
void CustomGraphics::mousePressEvent(QMouseEvent *e)
{
    ...
    if (tempPoints.isEmpty()) {
        ...
        selectedEllipseIndex = -1;
        //20221203增加tempPolygons.size()>0，否则加入图形调整后，删除再画会出错
        if (selectedIndex != -1 && tempPolygons.size()>0)
        {
...
void CustomGraphics::dispPoly(QList<QPoint> listPoint)
{
    if (listPoint.size() >= 3)
    {
        Polygon pol;
        pol.pos = listPoint.toVector();
        pol.selected = false;
        tempPolygons.append(pol);
    }
...
void CustomGraphics::getPoints()
{
    foreach (Polygon polygon, tempPolygons) {
        emit receivePoints(polygon.pos);
    }
}
